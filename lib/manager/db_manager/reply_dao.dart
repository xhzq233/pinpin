...
@dao
abstract class ReplyDao with BasicDao<Reply> {

  @Query('SELECT * FROM Reply WHERE pinPinId = :id')
  Future<List<Reply>?> getRepliesByPinPinID(int id);

  @Query('SELECT * FROM Reply WHERE pinPinId = :pinpinId and replyTo = :replyTo')
  Future<List<Reply>?> getRepliesByReplyTo(int pinpinId, int replyTo);
}
